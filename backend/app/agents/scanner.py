...
@track_agent(
    name="Scanner Agent  ",
    agent_type="tool",
    tags=["scanning", "multi-language", "tree-sitter", "agent"]
)
async def scan_codebase(state: AnalysisState) -> AnalysisState:
    """
    Agent 2: Scanner (ENHANCED WITH TREE-SITTER)
    
    Now supports 15+ languages with proper AST analysis
    """
    
    state["current_step"] = "scanner"
    state["progress"] = get_progress_for_step("scanner")
    
    print(f"🔍 [Scanner Agent  ] Starting multi-language analysis...")
    ...
    try:
        ...
        try:
            ...
            error_msg = f"Failed to import engine_ .py: {str(e)}"
            print(f"❌ [Scanner Agent  ] {error_msg}")
            state["errors"].append(error_msg)
            return state
        ...
        # ====================================================================
        # CALCULATE NCrF WITH TREE-SITTER
        # ====================================================================
        try:
        ...
        try:
            ...
            # Merge with API-detected markers from Validator
            ...
            print(f"   - Uses OOP/Interfaces: {sfia_markers.get('uses_design_patterns', False)}")
            print(f"   - Uses Async: {sfia_markers.get('uses_async', False)}")
            print(f"   - Error Handling: {sfia_markers.get('has_error_handling', False)}")
        ...
        print(f"✅ [Scanner Agent  ] Multi-language analysis complete")
