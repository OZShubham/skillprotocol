...
"""
Example 1: Track an agent

from app.tools.opik_logger import track_agent
...
Example 2: Track LLM calls in Grader Agent

from app.tools.opik_logger import track_llm_call
...
await track_llm_call(
    ...
    metadata={
        ...
        "sfia_level": parsed_response["sfia_level"]
    }
)


Example 3: Log routing decisions in graph.py

from app.tools.opik_logger import log_decision_point

def should_proceed_to_scanner(state):
    if state["validation"]["is_valid"]:
        log_decision_point(
            decision_name="proceed_to_scanner",
            input_data={"validation": state["validation"]},
            output_decision="scanner",
            reasoning="Validation passed, proceeding to scan"
        )
        return "scanner"
    else:
        log_decision_point(
            decision_name="skip_to_reporter",
            input_data={"validation": state["validation"]},
            output_decision="reporter",
            reasoning="Validation failed, skipping analysis"
        )
        return "reporter"


Example 4: Full workflow tracking

from app.tools.opik_logger import OpikContextManager

async def run_analysis(repo_url, user_id, job_id):
    async with OpikContextManager(job_id, repo_url) as tracker:
        
        tracker.log_step("validation", {
            "input": {"repo_url": repo_url},
            "output": validation_result
        })
        
        tracker.log_step("scanning", {
            "input": {},
            "output": scan_metrics
        })
