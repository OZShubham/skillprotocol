...
import { motion, AnimatePresence } from 'framer-motion'
import { Award, ExternalLink, Download, Share2, CheckCircle, Code, Shield, BrainCircuit, Check, ThumbsUp, ThumbsDown, MessageSquare, TrendingUp, Activity } from 'lucide-react'
import confetti from 'canvas-confetti'
import { useEffect, useState } from 'react'
import { api } from '../services/api' 
import OpikInsights from './OpikInsights'
      ...
      <div className="max-w-5xl w-full">
        ...
        >
          ...
          <div className="p-8 md:p-10 space-y-10">
            ...
            {/* 6. SINGLE Human Verification (Fixed) */}
            <FeedbackSection jobId={result.verification_id} />

            {/* 7. Opik Trace + Dashboard Links */}
            <div className="space-y-4">
              ...
              {/* NEW: Opik Dashboard Link */}
              <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-blue-500/10 rounded-lg flex items-center justify-center flex-shrink-0">
                    <Activity className="w-5 h-5 text-blue-400" />
                  </div>
                  <div>
                    <h4 className="text-white font-bold mb-1">Live Quality Dashboard</h4>
                    <p className="text-sm text-text-muted max-w-md">
                      See real-time accuracy metrics, A/B test results, and improvement trends.
                    </p>
                  </div>
                </div>
                <a 
                  href="/dashboard"
                  className="flex items-center gap-2 bg-blue-500 text-white hover:bg-blue-600 px-6 py-3 rounded-lg font-bold transition-colors shadow-lg shadow-blue-500/20 whitespace-nowrap"
                >
...
// --- FEEDBACK COMPONENT (FIXED - SINGLE INSTANCE) ---
function FeedbackSection({ jobId }) {
