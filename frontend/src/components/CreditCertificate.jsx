...
import { Award, ExternalLink, Download, Share2, CheckCircle, Code, Shield, BrainCircuit, Check, ThumbsUp, ThumbsDown, MessageSquare, TrendingUp, Activity, Target, AlertCircle } from 'lucide-react'
import confetti from 'canvas-confetti'
import { useEffect, useState } from 'react'
import { api } from '../services/api'

export default function CreditCertificate({ result, onViewDashboard }) {
  const [opikStats, setOpikStats] = useState(null)
  const [loadingOpik, setLoadingOpik] = useState(true)
  ...
  useEffect(() => {
    ...
    // Fetch real Opik metrics
    const fetchOpikMetrics = async () => {
      try {
        const data = await api.getOpikStats()
        setOpikStats(data)
      } catch (e) {
        console.error('Failed to load Opik metrics:', e)
      } finally {
        setLoadingOpik(false)
      }
    }
    fetchOpikMetrics()
  }, [])
      ...
      <div className="max-w-5xl w-full">
        ...
        >
          ...
          <div className="p-8 md:p-10 space-y-10">
            ...
            {/* OPIK QUALITY INSIGHTS - REAL DATA */}
            {!loadingOpik && opikStats && (
              <div className="p-6 bg-gradient-to-br from-[#0F172A] to-[#1E1B4B] border border-blue-500/20 rounded-xl">
                <div className="flex items-center gap-2 mb-6">
                  <Activity className="w-5 h-5 text-blue-400" />
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider">
                    Opik Quality Insights
                  </h3>
                </div>

                <div className="grid md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-black/30 border border-white/10 rounded-lg p-4">
                    <div className="text-xs text-text-dim mb-1">ACCURACY IMPROVEMENT</div>
                    <div className="text-2xl font-bold text-primary">
                      +{opikStats.improvement_percentage?.toFixed(1) || 0}%
                    </div>
                  </div>
                  <div className="bg-black/30 border border-white/10 rounded-lg p-4">
                    <div className="text-xs text-text-dim mb-1">CURRENT ACCURACY</div>
                    <div className="text-2xl font-bold text-green-400">
                      {(opikStats.current_accuracy * 100)?.toFixed(1) || 0}%
                    </div>
                  </div>
                  <div className="bg-black/30 border border-white/10 rounded-lg p-4">
                    <div className="text-xs text-text-dim mb-1">TOTAL ANALYSES</div>
                    <div className="text-2xl font-bold text-white">
                      {opikStats.total_analyses || 0}
                    </div>
                  </div>
                </div>

                {opikStats.ab_test_results && (
                  <div className="bg-black/20 border border-white/5 rounded-lg p-4">
                    <div className="text-xs text-text-dim mb-2">A/B TEST WINNER</div>
                    <div className="text-sm text-white">
                      {opikStats.ab_test_results.winner === 'b' ? 'Variant B (Optimized)' : 'Variant A (Baseline)'} - 
                      <span className="text-primary ml-1">
                        {opikStats.ab_test_results.variant_b?.success_rate 
                          ? `${(opikStats.ab_test_results.variant_b.success_rate * 100).toFixed(1)}%` 
                          : 'N/A'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-text-muted max-w-md">
                        View the immutable trace of the AI agents' decision-making process.
                      </p>
                    </div>
                  </div>
                  <a 
